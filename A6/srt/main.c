...
void* render(void* thread_num){
    ...
    for( int px=*(int*) thread_num; px<width; px += nthreads )
    {
        ...
        for( int py=0; py<height; ++py )
        {
            ...
            output[px * width * 3 + py * 3 + 0] = scaled_color[0];
            output[px * width * 3 + py * 3 + 1] = scaled_color[1];
            output[px * width * 3 + py * 3 + 2] = scaled_color[2];
        }
        //printf( "\n" );
    }
    return NULL;
...
main( int argc, char **argv )
{
    ...
    for (int id = 0; id < nthreads; id++){
        thread_id[id] = id;
        if (pthread_create(&threads[id], NULL, render, (void*) &thread_id[id])){
    ...
    for (int id = 0; id < nthreads; id++){
        if (pthread_join(threads[id], NULL)){
            fprintf(stderr, "Error ending threads");
            exit(1);
        }
    }
    for (int pos = 0; pos < size; pos += 3){
        printf( "%.0f %.0f %.0f\n", output[pos], output[pos + 1], output[pos + 2]);
        if ((pos + 3) % (width * 3) == 0)
            printf("\n");
    }
